  ...
  return (
    <div className="space-y-4">
          ...
          {images.map((img, i) => (
            <div key={img.id} className="rounded-lg border p-4">
              <div className="relative mb-3">
                <img src={img.url} alt={`Croqui ${i + 1}`} className="w-full cursor-pointer rounded object-contain" style={{ maxHeight: '500px' }} onClick={() => setAnnotatingIndex(i)} title="Clique para editar/anotar" />
                <div className="absolute right-2 top-2 flex gap-1">
                  <button className="flex h-7 w-7 items-center justify-center rounded bg-black/60 text-white hover:bg-black/80" onClick={e => { e.stopPropagation(); setLightbox(img.url); }} title="Ampliar"><ZoomIn className="h-4 w-4" /></button>
                </div>
                <button className="absolute left-2 top-2 rounded bg-primary p-1 text-primary-foreground hover:bg-primary/80" onClick={e => { e.stopPropagation(); setAnnotatingIndex(i); }} title="Anotar imagem"><Pencil className="h-4 w-4" /></button>
              </div>
              <div className="flex items-end gap-2">
                <div className="flex-1">
                  <Label className="text-xs">Legenda</Label>
                  <Input value={img.legenda} onChange={e => handleLegendaChange(img.id, e.target.value)} placeholder="Ex: Croqui de localização" />
                </div>
                <Button variant="ghost" size="icon" className="text-destructive" onClick={() => handleRemove(img.id)}><Trash2 className="h-4 w-4" /></Button>
              </div>
            </div>
          ))}
        </div>
      ...
      {annotatingIndex !== null && images[annotatingIndex] && (
        <ImageAnnotator imageUrl={images[annotatingIndex].url} onCancel={() => setAnnotatingIndex(null)} onSave={(dataUrl) => { onUpdate(images.map((img, i) => i === annotatingIndex ? { ...img, url: dataUrl } : img)); setAnnotatingIndex(null); }} />
      )}
    </div>
  );
}

...
async function pdfToImages(file: File): Promise<string[]> {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    const { data: urlData } = (await import('@/integrations/supabase/client')).supabase.storage.from('laudo-fotos').getPublicUrl(data.path);
...
export function DocumentacoesSection({ documentacoes, onUpdate, richText, onRichTextUpdate }: DocumentacoesSectionProps) {
  ...
  const handleRemoveDoc = (id: string) => onUpdate(documentacoes.filter(d => d.id !== id));
  const handleNomeChange = (id: string, nome: string) => onUpdate(documentacoes.map(d => d.id === id ? { ...d, nome } : d));
    ...
    <div className="space-y-4">
      <div className="a4-page mb-4">
        ...
        <div className="mt-6 flex justify-center">
          <Button variant="outline" className="gap-2" onClick={handleAddDoc}><Plus className="h-4 w-4" />Adicionar Documentação</Button>
        </div>
      </div>
      ...
      {annotating && (
        <ImageAnnotator
          ...
          onSave={(dataUrl) => {
            ...
            onUpdate(documentacoes.map(d => d.id === docId ? { ...d, imagens: d.imagens.map((url, i) => i === imgIndex ? dataUrl : url) } : d));
            setAnnotating(null);
          }}
        />
      )}
